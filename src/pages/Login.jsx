      ...
      >
        <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
          ...
          <p style={{ color: 'var(--text-muted)' }}>
            {isRegister ? 'Join us to manage your appointments.' : 'Please enter your details to sign in.'}
          </p>
        </div>

        {error && (
          <div style={{ background: 'rgba(239, 68, 68, 0.1)', color: 'var(--danger)', padding: '1rem', borderRadius: 'var(--radius-md)', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.9rem' }}>
          ...
          <AnimatePresence mode='wait'>
            {isRegister && (
              ...
              >
                ...
                <input type="text" className="form-control" placeholder="John Doe" required value={name} onChange={(e) => setName(e.target.value)} />
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div style={{ position: 'relative', margin: '2rem 0', textAlign: 'center' }}>
          <span style={{ position: 'relative', zIndex: 1, background: 'var(--bg-card)', padding: '0 1rem', fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            Or continue with
          </span>
        ...
        <div style={{ display: 'flex', gap: '1rem' }}>
          <button type="button" className="btn btn-outline" style={{ flex: 1, border: '1px solid var(--border-light)', display: 'flex', justifyContent: 'center' }}>
          ...
          <button type="button" className="btn btn-outline" style={{ flex: 1, border: '1px solid var(--border-light)', display: 'flex', justifyContent: 'center' }}>
            <svg style={{ width: '18px', height: '18px', fill: 'var(--text-main)' }} viewBox="0 0 384 512">
