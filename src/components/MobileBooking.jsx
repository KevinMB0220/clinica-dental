...
export default function MobileBooking() {
  ...
  const [formData, setFormData] = useState({ name: '', phone: '', specialty: '', date: '', time: '' });
    ...
    return (
      <div className="block lg:hidden" style={{ minHeight: 'calc(100vh - 90px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem 1.5rem', background: 'var(--bg-page)' }}>
        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} style={{ background: 'white', padding: '2.5rem 1.5rem', borderRadius: '32px', textAlign: 'center', boxShadow: '0 10px 40px rgba(0,0,0,0.05)', width: '100%' }}>
          ...
          <h2 style={{ fontSize: '1.6rem', fontWeight: 900, marginBottom: '0.5rem' }}>{t('booking.mobileSuccess')}</h2>
          <p style={{ color: 'var(--text-muted)', marginBottom: '2rem', fontSize: '0.95rem' }}>{t('booking.mobileSuccessDesc')}</p>
          <button onClick={() => navigate('/')} className="btn btn-primary" style={{ width: '100%', padding: '1rem' }}>{t('booking.mobileGoHome')}</button>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="block lg:hidden" style={{ background: 'var(--bg-page)', minHeight: '100vh', padding: '2rem 1.5rem 6rem' }}>
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <h1 style={{ fontSize: '2.5rem', fontWeight: 900, marginBottom: '0.5rem', lineHeight: 1.1 }}>
          {t('booking.mobileTitle')} <br />
          <span style={{ color: 'var(--primary)' }}>{t('booking.mobileTitleHighlight')}</span>
        </h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '1rem', marginBottom: '1.5rem' }}>
          {t('booking.mobileSubtitle')}
        </p>
        ...
        <form onSubmit={handleSubmit} style={{ background: 'white', padding: '1.5rem', borderRadius: '24px', boxShadow: '0 10px 40px rgba(0,0,0,0.03)', border: '1px solid var(--border-light)' }}>
          <h3 style={{ fontSize: '1.2rem', fontWeight: 800, marginBottom: '1.5rem' }}>{t('booking.mobileFormTitle')}</h3>

          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>{t('booking.fields.name')}</label>
              <input type="text" required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} placeholder={t('booking.fields.namePlaceholder')} style={{ padding: '0.85rem', borderRadius: '12px', border: '1px solid var(--border-light)', background: 'var(--bg-input)', fontSize: '16px' }} />
            </div>

            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>{t('booking.fields.phone')}</label>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
              <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>{t('booking.fields.specialty')}</label>
              <select value={formData.specialty || (Array.isArray(specialties) ? specialties[0] : '')} onChange={e => setFormData({...formData, specialty: e.target.value})} style={{ padding: '0.85rem', borderRadius: '12px', border: '1px solid var(--border-light)', background: 'var(--bg-input)', fontSize: '16px', appearance: 'none' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>{t('booking.fields.date')}</label>
              ...
              <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                <label style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-muted)' }}>{t('booking.fields.time')}</label>
          ...
          <button type="submit" disabled={loading} style={{ width: '100%', padding: '1rem', background: 'var(--primary)', color: 'white', border: 'none', borderRadius: '100px', fontWeight: 800, fontSize: '1rem', marginTop: '2rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
            {loading ? t('booking.mobileSending') : t('booking.mobileConfirm')} <ChevronRight size={18} />
          </button>
        </form>
      </motion.div>
    </div>
